...
# Directory Structure:
  ...
  - __conf/init.replacements.csv__ copy from `init.replacements.example.csv`, this is  an example file for the Track Correction csv files. You can import them via the Admin-cli interface. Regular Expression Groups can be used to define replacements for a specific part of a track e.g. remove [unnknown] from the Song Titles, extract Artist name from Title etc.
