	...
	<table class="pure-table pure-table-horizontal" id="playlistdata">
		...
		<tbody class="cmenu_default">
			...
			<tr onclick="setPlaylist(PLAYLIST.DEFAULT);loadSong($(this));"
				...
				onmouseover="activeRow(this);" onmouseout="inactiveRow(this);">
				<td>{if
					$autostart==true&&($track_count-1)=={$startvideo['videoIndex']}} <img
					src="./images/equalizer.gif" alt="equalizer" width="20" height="15" />
					{assign var="current_play" value="track_{$track_count}"} {else}
					&nbsp; {/if}
				</td> {if $track['video_id'] != -1}
				<td class="title_playing">{$track_count}*</td>
				<td class="title_playing">{$track['artist']}</td>
				<td class="title_playing">{$track['title']}</td>
				<td class="title_playing">{$track['dateofplay']}</td> {else}
				<td>{$track_count}</td>
				<td>{$track['artist']}</td>
				<td>{$track['title']}</td>
