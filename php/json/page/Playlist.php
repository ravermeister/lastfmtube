...
class Playlist extends DefaultJson {
	...
	private function getTopSongs($pageNum = 1) {
		...
		$page ['TRACKS'] = array ();
		if ($topsongs === 0 || ! is_array ( $topsongs ))
			return $page;
		$uniqueTracks = array ();
		...
		$uniqueTracks = usort(array_values($uniqueTracks), Functions::getInstance()->sortArrayByPlayCountDesc);		
