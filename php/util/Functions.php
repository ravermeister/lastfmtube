...
class Functions {
	...
	private function initSettings($force = false) {
		if (! $force && is_array ( $this->settings ))
			return;

		if ($this->settingsFile === false)
			$this->settingsFile = $this->basedir . '/conf/settings.json';
	...
	public function formatDate($date, $srcFormat = 'Y-m-d H:i:s') {
		...
		if (strcmp ( $srcFormat, $newFormat ) == 0)
			return $date;
			...
			foreach ( $msg as $item ) {
				...
				for($i = 0; $i < sizeof ( $msgArr ); $i ++) {
					if (strlen ( $msgArr [$i] ) > 0)
						fwrite ( $this->logFile, $prefix . "\t" . $msgArr [$i] . "\r\n" );
				}
			}
		} else {
			$msgArr = explode ( "\n", self::br2nl ( $msg ) );
	...
	private static function sortArrayByPlayCountAsc($trackA, $trackB) {
		...
		Functions::getInstance()->logMessage('acnt='.$aCnt.' bcnt='.$bCnt);
