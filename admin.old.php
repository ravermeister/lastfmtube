...
if (isset ($_POST ['submit']) && strcmp($_POST['submit'], 'Save') == 0) {
    ...
    if (strlen(trim(strip_tags($_POST ['general_baseurl']))) >
        0) $settings ['general'] ['baseurl'] = $_POST ['general_baseurl'];
    if (strlen(trim(strip_tags($_POST ['general_lang']))) > 0) $settings ['general'] ['lang'] = $_POST ['general_lang'];
    if (strlen(trim(strip_tags($_POST ['general_logpath']))) >
        0) $settings ['general'] ['logpath'] = $_POST ['general_logpath'];
    if (strlen(trim(strip_tags($_POST ['general_playerwidth']))) >
        0) $settings ['general'] ['playerwidth'] = $_POST ['general_playerwidth'];
    if (strlen(trim(strip_tags($_POST ['general_playerheight']))) >
        0) $settings ['general'] ['playerheight'] = $_POST ['general_playerheight'];

    if (strlen(trim(strip_tags($_POST ['general_theme']))) >
        0) $settings ['general'] ['theme'] = $_POST ['general_theme'];
    if (strlen(trim(strip_tags($_POST ['general_cmenutheme']))) >
        0) $settings ['general'] ['cmenutheme'] = $_POST ['general_cmenutheme'];
    if (strlen(trim(strip_tags($_POST ['general_lastfm_defaultuser']))) >
        0) $settings ['general'] ['lastfm_defaultuser'] = $_POST ['general_lastfm_defaultuser'];
    // general settings

    if (strlen(trim(strip_tags($_POST ['database_dsn']))) > 0) $settings ['database'] ['dsn'] = $_POST ['database_dsn'];
    if (strlen(trim(strip_tags($_POST ['database_table_prefix']))) >
        0) $settings ['database'] ['table_prefix'] = $_POST ['database_table_prefix'];
    if (strlen(trim(strip_tags($_POST ['database_username']))) >
        0) $settings ['database'] ['username'] = $_POST ['database_username'];
    if (strlen(trim(strip_tags($_POST ['database_password']))) >
        0) $settings ['database'] ['password'] = $_POST ['database_password'];
    // database settings

    if (strlen(trim(strip_tags($_POST ['lastfm_user']))) > 0) $settings ['lastfm'] ['user'] = $_POST ['lastfm_user'];
    if (strlen(trim(strip_tags($_POST ['lastfm_apikey']))) >
        0) $settings ['lastfm'] ['apikey'] = $_POST ['lastfm_apikey'];
    // lastfm

    if (strlen(trim(strip_tags($_POST ['youtube_apikey']))) >
        0) $settings ['youtube'] ['apikey'] = $_POST ['youtube_apikey'];
    if (strlen(trim(strip_tags($_POST ['youtube_email']))) >
        0) $settings ['youtube'] ['email'] = $_POST ['youtube_email'];
    if (strlen(trim(strip_tags($_POST ['youtube_keyfile']))) >
        0) $settings ['youtube'] ['keyfile'] = $_POST ['youtube_keyfile'];
...
$mergeTracks[] = array('display_artist' => 'dummy disp artist', 'display_track' => 'dummy disp track',
                       'merge_artist'   => 'dummy merge artist', 'merge_track' => 'dummy merge track',
                       'display_amount' => 'display amount', 'merge_amount' => 'merge amount',);
